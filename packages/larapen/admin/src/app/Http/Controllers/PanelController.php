...
class PanelController extends Controller
{
	...
	public function edit($id, $childId = null)
	{
		...
		$this->data['entry'] = (isset($entry) && !empty($entry)) ? $entry : $this->xPanel->getEntry($id);
                ...
                $this->xPanel->entry = $this->data['entry'];

		$this->data['xPanel'] = $this->xPanel;
		$this->data['saveAction'] = $this->getSaveAction();
		$this->data['fields'] = $this->xPanel->getUpdateFields($id);
		$this->data['title'] = trans('admin::messages.edit') . ' ' . $this->xPanel->entry->name;

		$this->data['id'] = $id;

//echo(json_encode($this->data['xPanel'], JSON_PRETTY_PRINT));
	...
	public function updateCrud(UpdateRequest $request = null)
	{
		try {
			...
			if(!is_null($item->contact_name) && $item->user_id != 0){	
				NotifyController::sendMesageFromAdmin($item, $notify);
			}

    ...
    $.ajax({
        ...
        if (data.status == 1) {
            if ($(elmt).hasClass('btn')) {
                ...
                $(elmt).html('<i class="unir-heart gray" data-toggle="tooltip" data-original-title="' 
                + lang.labelSavePostRemove + '"></i>&nbsp&nbsp<span class="actions-text">' + lang.labelSavePostRemove + ' </span> ');
                $("#make_favorite").empty()
                $("#make_favorite").prepend('<div class="alert alert-success" role="alert">'+lang.confirmationSavePost+'</div>');
            }
        } else {
            if ($(elmt).hasClass('btn')) {
                $('#' + data.postId).removeClass('btn-success').addClass('btn-default');
            } else {
                $(elmt).html('<i class="unir-bheart gray" data-toggle="tooltip" data-original-title="' + lang.labelSavePostSave 
                + '"></i>&nbsp&nbsp</i<span class="actions-text">' +  lang.labelSavePostSave + ' </span>');
                $("#make_favorite").empty();
                $("#make_favorite").prepend('<div class="alert alert-success" role="alert">'+lang.confirmationRemoveSavePost+'</div>');
            }
        }

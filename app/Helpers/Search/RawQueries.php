...
class RawQueries
{
	...
	public function fetch()
	{
		...
		// var_dump($posts);
		//delete only from view ,posts who owner is banned and phone is in blacklist table
		// R.S.
		// foreach($posts as $post){
		// 	$ownerId = "SELECT user_id FROM " . $this->table . " WHERE id= " . $post->id . ";"; 
		// 	$owner = self::execute($ownerId);
			
		// 	$isBanned = self::execute("select id from .blacklist where entry = (select phone from users where id = '{$owner[0]->user_id}')") ;
		// 	if($isBanned){
		// 		// var_dump($owner[0]->user_id);
		// 		// var_dump(array_search($post,$posts));
	...
	public function setPrimaryFilters()
	{
		...
		if (request()->has('location') && !empty($this->city)) {
	...
	public function setCategory($catId, $subCatId = null)
{
    ...
    public function setNarrowedCategory($list)
    {
        if ($list) {
            $this->arrSql->where['tPost.category_id'] = ' IN (' . implode(",", $list) . ")";
        }
			...
			if ($key == 'cf') { // Custom Fields
				if (is_array($value)) {
					...
					foreach ($value as $fieldId => $postValue) {
                        ...
                        // TODO N.S. Для дерева работает логика OR, не AND
                        $checkField = Field::findTrans($fieldId);
                        if ($checkField->type=='tree' || $checkField->type=='checkbox_multiple_or') {

                            $bindId = 'tree'. (int)$fieldId;
                            $alias = 'av' . $bindId;
                            $optionIN = [];
                            $valueIN = [];
                            ...
                            $where = '('
                                ...
                                . ' AND ' . $alias . '.option_id IN ('.implode(",", $optionIN).')'
                                . ' AND ' . $alias . '.value IN (' . implode(",", $valueIN) . ')'
                                . ')';
                        ...
                        elseif ($checkField->type=='number') {

                            $bindId = 'number'. (int)$fieldId;
                            $alias = 'av' . $bindId;

                            // Min and max values
                            if ($fieldId) {
                                $placeholderValue = DB::select('SELECT MIN(value) as min, MAX(value) as max FROM post_values WHERE field_id = ' . $fieldId);
                            }
                            else {
                                $placeholderValue = DB::select('SELECT MIN(price) as min, MAX(price) as max FROM posts');
                            }

                            if (!$postValue[0]) $minValue = $placeholderValue[0]->min;
                            else $minValue = $postValue[0];

                            if (!$postValue[1]) $maxValue = $placeholderValue[0]->max;
                            else $maxValue = $postValue[1];

                            $where = '('
                                . $alias . '.field_id = :fieldId' . $bindId
                                . ' AND ' . $alias . '.value BETWEEN '. $minValue . ' AND ' . $maxValue
                                . ')';
